<?php
    function carrega_pagina($con, $data, $isAdmin){
        ...
        if(file_exists($dir.$explode['0'].$ext)){
            ...
            echo "<div class='alert'>Página não encontrada </div>";
        }
    }
    ...
    function protegePagina($explode, $isAdmin){
        $paginas_protegidas = array("inicio", "publicar","publicacoes","gerenciar-posts","editar-post","deletar-post","perfil");

        if(!isset($_SESSION['usuarioID']) && in_array($explode, $paginas_protegidas) && $isAdmin == true){
            redireciona('login',false, false, 0);
            exit();
        }
        if(isset($_SESSION['usuarioID']) && $explode == "login"){
            redireciona('inicio', false, false, 0);
        }
    }
